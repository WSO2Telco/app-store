 ...
 ******************************************************************************/
package org.appstore.core.authfilter.authorization;
...
import org.appstore.core.authfilter.impl.authorization.BearerAuthenticationFilter;

public class UserAuthorizationValidator {

	private final Log log = LogFactory.getLog(UserAuthorizationValidator.class);
    private static final Logger logger = Logger.getLogger(UserAuthorizationValidator.class.getName());
	...
	public Boolean isAuthorizedScope(String[] currentTokenScopes, Set<String> allowedScopeSet){
		...
		logger.log(Level.INFO, "isAuthorized 6 ");
		List<String> currentTokenScopesList = Arrays.asList(currentTokenScopes);

		logger.log(Level.INFO, "isAuthorized 7 ");
		Iterator<String> iterator = allowedScopeSet.iterator();
		while (iterator.hasNext()) {

			String allowedScope = iterator.next();
			logger.log(Level.INFO, "isAuthorized 8 allowedScope "+allowedScope +" currentTokenScopes"+currentTokenScopes);
